...
module.exports = function destroyOneRecord (req, res) {
  ...
  query.exec(function foundRecord (err, record) {
    ...
    if(!record) return res.notFound('No record found with the specified `id`.');
		...
		// check if the model have a deleted flag
    if (_.has(Model, 'definition.isDeleted')) {
    	var options = {
			...
			if (req.user) {
				if (_.has(Model, 'definition.deletedBy')) {
					options.deletedBy = req.user.id;
				}

				if (_.has(Model, 'definition.deletedAt')) {
					options.deletedAt = new Date;
				}
			}

			Model.update(pk, options).exec(function updated(err, updatedRecord) {
